...
public final class BooleanExpressionEvaluator {
    ...
    public static boolean valueOfBoolExpr(Queue<String> tokens) {
        ...
        if (tokens.length() == 2
                && (tokens.front().equals("T") || tokens.front().equals("F"))) {
            front = tokens.dequeue();
            if (front.equals("F")) {
                value = false;
            } else if (front.equals("T")) {
                value = true;
            }
        } else {
            front = tokens.dequeue();
            ...
            } else if (front.equals("(") || front.equals(" ")) {
                boolean leftValue = valueOfBoolExpr(tokens);
                String binaryOp = tokens.dequeue();
                boolean rightValue = valueOfBoolExpr(tokens);
