...
//        T qEntry;
//        Queue<T> largerQueue = q.newInstance(); // split queue into two
//        while (largerQueue.length() != 0 || order.compare(x, qEntry) >= 0) {
//            qEntry = q.dequeue();
//            largerQueue.enqueue(qEntry);
//        }
//        q.enqueue(qEntry);
//        q.append(largerQueue);

        //find something larger than x
//        Queue<T> smallQ = q.newInstance();
//        boolean largerThanX = false;
//        while (!largerThanX) {
//            T entry = q.dequeue();
//            largerThanX = (order.compare(x, entry) > 0);
//            q.enqueue(entry);
//        }
...
//        q.transferFrom(newQ);
    ...
    public void sort(Comparator<T> order) {
        assert order != null : "Violation of: order is not null";
//        T lastElement = this.front();
//        int length = this.length();
//        for (int i = 0; i <= length; i++) {
//            T element = this.dequeue();
//            if (order.compare(lastElement, element) >= 0) {
//
//            } else {
...
//        }
        ...
        for (int i = 0; i <= length; i++) {
            ...
            insertInOrder(newQ, element, order);
        }
