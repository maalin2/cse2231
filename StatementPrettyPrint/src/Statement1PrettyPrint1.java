    ...
    public void prettyPrint(SimpleWriter out, int offset) {
        ...
        switch (this.kind()) {
            case BLOCK: {
                ...
                for (int i = 0; i < len; i++) {
                    Statement st = this.removeFromBlock(i);
                    st.prettyPrint(out, offset);
            ...
            case IF: {
                ...
                Condition cond = this.disassembleIf(block);

                String condStr = toStringCondition(cond);
                printSpaces(out, offset);

                out.print("IF " + condStr + " THEN\n");
                block.prettyPrint(out, offset + Program.INDENT_SIZE);

                printSpaces(out, offset);
                out.print("END IF\n");

                this.assembleIf(cond, block);
            ...
            case IF_ELSE: {
                ...
                Condition cond = this.disassembleIfElse(ifBlock, elseBlock);
                String condStr = toStringCondition(cond);

                printSpaces(out, offset);
                out.print("IF " + condStr + " THEN\n");
                ifBlock.prettyPrint(out, offset + Program.INDENT_SIZE);

                printSpaces(out, offset);
                out.print("ELSE\n");
                elseBlock.prettyPrint(out, offset + Program.INDENT_SIZE);

                printSpaces(out, offset);
                out.print("END IF\n");

                this.assembleIfElse(cond, ifBlock, elseBlock);
            ...
            case WHILE: {
                ...
                Condition cond = this.disassembleWhile(block);
                String condStr = toStringCondition(cond);

                printSpaces(out, offset);
                out.print("WHILE " + condStr + " DO\n");
                block.prettyPrint(out, offset + Program.INDENT_SIZE);

                printSpaces(out, offset);
                out.print("END WHILE\n");

                this.assembleWhile(cond, block);

                break;
            }
            case CALL: {
                String inst = this.disassembleCall();
                printSpaces(out, offset);
                out.print(inst + "\n");
                this.assembleCall(inst);
